...
/* ─── Quote callout whose left rule draws in as an SVG path on scroll ─── */
function QuoteWithDrawnLine({ children }: { children: React.ReactNode }) {
  const ref = useRef(null)
  const inView = useInView(ref, { once: true, margin: '-10% 0px' })
  return (
    <div ref={ref} style={{ position: 'relative', paddingLeft: '1.25rem', margin: '0 0 2rem' }}>
      <svg
        aria-hidden
        viewBox="0 0 2 100"
        preserveAspectRatio="none"
        style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '2px', height: '100%', overflow: 'visible' }}
      >
        <motion.line
      ...
      <p style={{ fontFamily: 'var(--font-dm-sans), DM Sans, sans-serif', fontSize: 'clamp(17px, 1.6vw, 21px)', color: '#fff', lineHeight: 1.55, margin: 0 }}>
        {children}
      </p>
    </div>
  )
      ...
      <FadeUp>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(2rem, 5vw, 5rem)', alignItems: 'center' }}>
          ...
          <div>
            ...
            <QuoteWithDrawnLine>{t.about.bio3Quote}</QuoteWithDrawnLine>
