...
export default function ProcessTimeline({ steps, dragHint }: { steps: Step[]; dragHint: string }) {
  ...
  return (
    <div>
      ...
      <div ref={gridRef} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : `repeat(${n}, 1fr)`, gap: isMobile ? '1rem' : '2px' }}>
        {steps.map((step, idx) => {
          const isFirst  = idx === 0
          ...
          return (
            <motion.div
              ...
              transition={{ duration: 0.65, ease: [0.25, 0.46, 0.45, 0.94], delay: idx * 0.09 }}
              style={{ height: '100%' }}
            >
              <motion.div
                onClick={() => jumpTo(idx)}
