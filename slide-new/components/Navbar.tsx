...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile menu — full-screen bold index overlay */}
      <AnimatePresence>
        {open && (
          <motion.div
            key="mobile-menu"
            ...
            style={{
              ...
              gap: '2rem',
              overflowY: 'auto',
              padding: 'calc(env(safe-area-inset-top, 0px) + 5.5rem) 6vw max(env(safe-area-inset-bottom, 0px), 2rem)',
            }}
          >
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(0.4rem, 2vw, 0.9rem)' }} aria-label="Mobile navigation">
              {NAV_ITEMS.map((item, i) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 12 }}
                  transition={{ delay: 0.16 + i * 0.055, duration: 0.5, ease: [0.25, 0.46, 0.45, 0.94] }}
            ...
            >
              <Link
                href="/kontakt"
                onClick={() => setOpen(false)}
                style={{ display: 'block', padding: '14px 18px', color: '#000', background: '#fff', fontSize: '15.41px', fontWeight: 500, textDecoration: 'none', borderRadius: '10px', marginBottom: '8px', textAlign: 'center' }}
              >
                {t.hero.cta}
              </Link>
              ...
              <Link
                ...
                rel="noopener noreferrer"
                onClick={() => setOpen(false)}
                style={{ display: 'block', padding: '12px 16px', color: 'rgba(255,255,255,0.72)', fontSize: '14px', fontWeight: 400, textDecoration: 'none', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)', marginBottom: '10px', textAlign: 'center' }}
              >
                {t.nav.login}
              </Link>
